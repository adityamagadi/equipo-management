...
from equipo.settings import MEDIA_ROOT, MEDIA_URL
...
def post(request, ptype, pk):
    """Display a post form."""
    action = reverse("equipo.forum.views.%s" % ptype, args=[pk])
    if ptype == "new_thread":
...
def new_thread(request, pk):
    ...
    return HttpResponseRedirect(reverse("equipo.forum.views.forum", args=[pk]))
...
def reply(request, pk):
    ...
    if p["body"]:
        ...
        increment_post_counter(request)
    return HttpResponseRedirect(reverse("equipo.forum.views.thread", args=[pk]) + "?page=last")
